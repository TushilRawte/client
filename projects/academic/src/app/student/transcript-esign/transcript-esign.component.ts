...
@Component({
  selector: 'app-transcript-esign',
  standalone: false,
  templateUrl: './transcript-esign.component.html',
  styleUrl: './transcript-esign.component.scss'
})
export class TranscriptEsignComponent implements OnInit {
  ...
  private createGenerateTrascriptForm(data: any[]) {
    ...
    data.forEach((row, index) => {
      const isDisabled =
        !(row.transcript_gen_yn === 'Y' && row.is_certificate_signed !== 'Y');
  ...
  async esignTrascript_btn() {
    const selectedRows = this.studentRowsFormArray.value.filter((r: any) => r.selected);
    // console.log("selectedRows ==sd=d==> ", selectedRows);

    if (selectedRows.length === 0) {
      this.alert.alertMessage("No Selection", "Please select at least one student OR Generate first transcript.", "error");
      return;
    }
    let { academic_session_id, degree_programme_id, college_id, ue_id } = this.getSRCDetailsForm.value
    ...
    // ! start------------------ esign call -------------------------------------------------------
    // moduleid => Transcript_UG = 3
    // moduleid => Transcript_PG_PhD = 4
    // ✅ Run setup ONCE before all colleges
    await this.esign.setupEDS(selectedDegreePro[0]?.degree_programme_type_id === 1 ? 3 : 4); // moduleid from table dsc_modules
    ...
    let selectedstudents: any = await Promise.all(
      selectedRows.map(async (student: any) => {
        ...
        return {
          ...
          transcript_gen_yn: student.transcript_gen_yn,
    ...
    this.http.postData('/studentProfile/post/transcriptEsign', { students: selectedstudents }, 'academic')
      .subscribe(
        (result: any) => {
          // console.log("transcriptEsign====<<<<S : ", result?.body);
          if (result?.body?.data?.message) {
            this.alert.alertMessage(result.body.data.message || "Trascript E-sign Done.", "", "success")
          } else if (result?.body?.error?.message) {
        ...
        (error) => {
          console.error('Error in transcriptEsign:', error);
          this.alert.alertMessage("Something went wrong!", "Network error occurred", "error");
        });
  }
