...
import { MobileNumberUpdatePopupComponent } from '../student-profile-update/mobile-number-update-popup/mobile-number-update-popup.component';
import { AddressUpdatePopupComponent } from '../student-profile-update/address-update-popup/address-update-popup.component';
import { NameUpdatePopupComponent } from '../student-profile-update/name-update-popup/name-update-popup.component';
...
export class StudentProfileComponent {
  ...
  selectedDegreeProgrammeId: number | null = null;
  ...
  getStudentDetails_Btn_click(actionType: string) {
    let { ue_id } = this.profileFormGroup.get('input_data')?.value;
    ...
    if (actionType === 'refresh') {
      ...
      this.getDegreeDetailsOptions.dataSource = [];
      this.getDegreeDetailsOptions.listLength = 0;
    }
        ...
        .subscribe(
          (result: any) => {
            ...
            if (result?.body?.data?.length === 0) {
              this.alert.alertMessage("Invalid User", "No Records Found in Database", "error");
              ...
              this.getDegreeDetailsOptions.dataSource = data;
              this.getDegreeDetailsOptions.listLength = data.length;
            }
          },
          (error) => {
  ...
  uploadImage(file: File, type: 'profile' | 'signature'): void {
    ...
    formData.append('student_id', ue_id.toString());
  ...
  getStudentProfileData(row: any) {
    this.selectedDegreeProgrammeId = row.degree_programme_id;
    ...
    this.http.getParam(
      ...
      .subscribe(
        (result: any) => {
          ...
          this.profileFormGroup.get('student_info')?.patchValue({
  ...
  onMobileNumberUpdate() {
    let { ue_id, mobile, e_mail, dob } = this.profileFormGroup.get('student_info')?.value
  ...
  isRowSelected(item: any): boolean {
    return this.selectedDegreeProgrammeId === item.degree_programme_id
  }


  clearSelection(): void {
    this.selectedDegreeProgrammeId = null;
