...
@Component({
  selector: 'app-marks-entry-thesis-unfinalize',
  standalone: false,
  templateUrl: './marks-entry-thesis-unfinalize.component.html',
  styleUrl: './marks-entry-thesis-unfinalize.component.scss'
})
export class MarksEntryThesisUnfinalizeComponent implements OnInit{
...
onUnFinalize() {
  const changedStudents = this.students.controls
    .filter(ctrl => ctrl.get('marks_finalize')?.dirty)
    .map(ctrl => {
      const val = ctrl.value;
      return {
        ...val,
        marks_finalize: val.marks_finalize ? 1 : 4 // ✅ true→1 (finalized), false→4 (unfinalized)
      };
    });

  if (changedStudents.length === 0) {
    this.alert.alertMessage("No changes detected!", "", "info");
    return;
  }

  console.log("Changed Finalize Payload:", changedStudents);

  const apiUrl = '/attendance/update/updateMarksFinalizethesis/';
  this.HTTP.putData(apiUrl, changedStudents, 'academic').subscribe(res => {
    if (!res.body.error) {
      this.alert.alertMessage("Finalize status updated successfully!", "", "success");
